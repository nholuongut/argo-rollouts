...
RUN curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/main/install.sh | sh -s -- -b $(go env GOPATH)/bin v1.57.2 && \
...
WORKDIR /go/src/github.com/nholuongut/argo-rollouts
...
COPY --from=argo-rollouts-build /go/src/github.com/nholuongut/argo-rollouts/dist/kubectl-argo-rollouts /bin/kubectl-argo-rollouts
...
COPY --from=argo-rollouts-build /go/src/github.com/nholuongut/argo-rollouts/dist/rollouts-controller /bin/
